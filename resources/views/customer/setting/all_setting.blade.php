@extends('layouts.customer_layout') {{-- Apne customer layout ka naam yahan likhein --}}
...
<div class="dashboard-body">
    ...
    @if (session('message'))
        <script>
            ...
            switch(type){
                case 'success': toastr.success("{{ session('message') }}"); break;
                case 'error': toastr.error("{{ session('message') }}"); break;
            }
        </script>
    @endif
    ...
    <div class="card overflow-hidden mt-20">
        <div class="card-header"><h4 class="card-title">Update Your Account Details</h4></div>
        <div class="card-body">
            <form method="POST" action="{{ route('customer.settings.update') }}" class="forms-sample">
                @csrf
                @method('PUT')
